...
void ShowTree(Node*& pNode, int L) // length from root to this peak
{
...
void PrefixOrder(Node*& pNode) // straight
{
...
void PostfixOrder(Node*& pNode) // reversed
{
    if (pNode == NULL) return;
...
void InfixOrder(Node*& pNode) //inner
{
    if (pNode == NULL) return;
...
bool SearchNodeBST(Node* pNode, datatype key) // BST - binary search tree
{
